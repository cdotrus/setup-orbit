...
# Project: setup-orbit
# Script: autocl
# Usage: python autocl.py <version>
# Steps:
...
#
# An automated changelog utlity helper script. It reads the project's
# changelog before releasing a new version to verify it is up to date and ready
# to extract the most recent version's information. If successful, prints the 
# information to stdout, else exits with error code and error message.
...
def main():
    if len(sys.argv) != 2:
        exit('error: expecting a single argument "<version>"')
